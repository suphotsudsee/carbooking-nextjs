...
export function TabSwitcher({ bookings, vehicles, drivers, users, canViewUsers }: Props) {
  ...
  const visibleTabs = useMemo(
    () => ["bookings", "vehicles", "drivers", "users"].filter((tab) => tab !== "users" || canViewUsers) as TabKey[],
    [canViewUsers],
  );
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="p-4">
        {active === "bookings" && (
          <BookingsList data={bookings} routerRefresh={router.refresh} onPrint={(booking) => setPrintBooking(booking)} />
        )}
        {active === "vehicles" && (
          <VehiclesList
            data={vehicles}
...
function BookingsList({
  ...
  if (!data.length) {
    return <EmptyState text="ยังไม่มีรายการจอง" />;
  }

  return (
    <div className="space-y-3">
      <ListHeader title="ข้อมูลการจองล่าสุด" subtitle="ตรวจสอบ อนุมัติ หรือจัดการรายการจองรถ" />
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="min-w-full text-sm text-slate-800">
          <thead className="bg-slate-50 text-slate-600">
            <tr className="text-left">
              <th className="px-4 py-3 font-semibold">ID</th>
              <th className="px-4 py-3 font-semibold whitespace-nowrap">หมายเลขจอง</th>
